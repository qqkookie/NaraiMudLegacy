...
*/
{
	...
	if (IS_SET(ch->specials.affected_by, AFF_PARALYSIS)) {
		send_to_char("You wanted to move, but you couldn't!\n\r",ch);
		return;
	}
    ...
    if ((world[ch->in_room].sector_type == SECT_WATER_NOSWIM) ||
        ...
        for (obj=ch->carrying; obj; obj=obj->next_content)
            if (obj->obj_flags.type_flag == ITEM_BOAT)
                has_boat = TRUE;
        if (!has_boat && (IS_NPC(ch) || GET_LEVEL(ch) != IMO+3)) {
	...
	// added by jmjeong, tunnel 처리
	if (IS_SET(world[world[ch->in_room].dir_option[cmd]->to_room].room_flags,
	TUNNEL) &&
	num_pc_in_room(&(world[world[ch->in_room].dir_option[cmd]->to_room])) >= tunnel_size) {
	    if (tunnel_size > 1)
...
void do_lock(struct char_data *ch, char *argument, int cmd)
{
    ...
    if (!*type) {
        ...
        else if (!has_key(ch, EXIT(ch, door)->key) && GET_LEVEL(ch)<IMO+2)
        ...
        else if (!has_key(ch, obj->obj_flags.value[2])&& GET_LEVEL(ch)<IMO+2)
