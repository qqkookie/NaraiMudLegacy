                 ...
                 struct char_data *tar_ch, struct obj_data *tar_obj)
{
    ...
    case SPELL_TYPE_SPELL:
        if (tar_obj) {        /* It's an object */
            if ( IS_OBJ_STAT(tar_obj, ITEM_BLESS) ) {
                send_to_char("Nothing seems to happen.\n\r", ch);
                return;
            }
                       ...
                       struct char_data *tar_ch, struct obj_data *tar_obj )
{
    ...
    case SPELL_TYPE_POTION:
        if ( affected_by_spell(ch, SPELL_DETECT_ALIGN) )
            return;
        spell_detect_align(level,ch,ch,0);
        break;
    case SPELL_TYPE_STAFF:
        for (tar_ch = world[ch->in_room].people ; 
             tar_ch ; tar_ch = tar_ch->next_in_room)
            if (tar_ch != ch) 
                if(!(IS_AFFECTED(tar_ch, SPELL_DETECT_ALIGN)))
                    spell_detect_align(level,ch,tar_ch,0);
        break;
    default : 
                        ...
                        struct char_data *tar_ch, struct obj_data *tar_obj )
{
    ...
    case SPELL_TYPE_SPELL:
        if (tar_obj) {
            if ( IS_OBJ_STAT(tar_obj, ITEM_INVISIBLE) )
...
void cast_cure_paralysis( byte level, struct char_data *ch, char *arg, int type,
                         struct char_data *tar_ch, struct obj_data *tar_obj )
{
    switch (type) {
    case SPELL_TYPE_SPELL:
        spell_cure_paralysis(level, ch, tar_ch, tar_obj);
        break;
    case SPELL_TYPE_POTION:
        spell_cure_paralysis(level, ch, ch, 0);
        break;
    case SPELL_TYPE_STAFF:
        for (tar_ch = world[ch->in_room].people ; 
             tar_ch ; tar_ch = tar_ch->next_in_room)
            if (tar_ch != ch) 
                spell_cure_paralysis(level,ch,tar_ch,0);
        break;
    default : 
        log("Serious screw-up in cure paralysis!");
        break;
    }
}
                 ...
                 struct char_data *tar_ch, struct obj_data *tar_obj )
{
    ...
    if (world[ch->in_room].dir_option[direction]->to_room < 0) {
...
void cast_charm_person( byte level, struct char_data *ch, char *arg, int type,
                        struct char_data *tar_ch, struct obj_data *tar_obj )
{
	extern int	nokillflag ;	/* nokill flag */

    switch (type) {
    case SPELL_TYPE_SPELL:
		/* If nokil flag is set, player can't charm player */
		if (!tar_ch) return ;
		if(!IS_MOB(tar_ch) && nokillflag ){
                           ...
                           int type, struct char_data *tar_ch, struct obj_data *tar_obj )
{
	...
	if ( *name == 0 ) {
		send_to_char("change weather to what ?\n\r", ch) ;
		return ;
    }

	if ( isname(name, "worse") ) {
		changes = -1 ;
    }
	else if ( isname(name, "better")) {
		changes = 1 ;
    }
	else {
		send_to_char("Say only  'worse' or 'better'.\n\r", ch) ;
		return ;
    }

	/* consider level.. */
	if ( GET_LEVEL(ch) > number(1,IMO)  )
		control_weather(changes) ;
}
