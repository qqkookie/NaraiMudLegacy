		    ...
		    struct char_data *victim, struct obj_data *obj)
{
	...
	/* NOTE: prohibit 'relo' to player of diffrent guild */
	else if ((i == 0 ) && !IS_NPC(victim) && GET_GUILD(victim) > 0 
	    && ( GET_GUILD(ch) != GET_GUILD(victim))) {
	    send_to_char("Your spell is not powerful enough for that.\n\r", ch);
	    return;
	}
		      ...
		      struct char_data *victim, struct obj_data *obj)
{
	...
	mana = GET_MOVE(victim) / 2 + number (1, GET_LEVEL(victim))*50;
	/* NOTE: More level advantage : 1000 for level 40 */
		 ...
		 struct char_data *victim, struct obj_data *obj)
{
	...
	/* NOTE: Curse will remove SANC affection */
	if (affected_by_spell(victim, SPELL_SANCTUARY)) {
	    affect_from_char(victim, SPELL_SANCTUARY);
	    act( spell_wear_off_msg[SPELL_SANCTUARY], 
		       ...
		       struct char_data *victim, struct obj_data *obj)
{
    ...
    INCREASE_SKILLED2(ch, victim, SPELL_DETECT_ALIGN);
			  ...
			  struct char_data *victim, struct obj_data *obj)
{
    ...
    struct obj_data *tmp;
    char buf[MAX_BUFSIZ];
    extern void enchant_weapon_special( struct char_data *ch,
    		struct obj_data *obj);
	...
	if (!IS_SET(obj->obj_flags.extra_flags, ITEM_MAGIC)) {
	    ...
	    /* NOTE: 1/1800 for hr * 4 : not changed */
	    ...
	    /* NOTE 1/4000 for dr*10  */
	    if (number(0, 4000) == 888) {
	    ...
	    /* NOTE 1/10000 for hr*10, dr*13  */
	    if (number(0, 10000) ==  1234) {
	    ...
	    /* NOTE 1/600 for special flag  */
	    if (number(0, 3000) < 5) {

		/* NOTE: code for adding weapon special magic is separated 
			as enchant_weapon_special() in "magic.c" */
		enchant_weapon_special(ch, obj);
		...
		if (number(0, 18000) == 900 || number(0, 18000) == 176) {
		    ...
		    /* NOTE: Log Thunderbolt enchant! */
		    sprintf(buf, "THUNDERBOLT!! by %s : %dD%d.", GET_NAME(ch),
			     ...
			     struct char_data *victim, struct obj_data *obj)
{
    ...
    if (!affected_by_spell(victim, SPELL_PROTECT_FROM_ALIGN)) {
	INCREASE_SKILLED2(ch, victim, SPELL_PROTECT_FROM_ALIGN);
	af.type = SPELL_PROTECT_FROM_ALIGN;
	af.duration = 24;
	af.modifier = 0;
	af.location = APPLY_NONE;
		     ...
		     struct char_data *victim, struct obj_data *obj)
{
    ...
    if (!affected_by_spell(victim, SPELL_SANCTUARY)) {
	/* NOTE: Cursed character can't get sanctuary */
	if (affected_by_spell(victim, SPELL_CURSE)) {
	    send_to_char("Huk! You are under bad influence! Damn it!\r\n", ch);
	    return;
	}
