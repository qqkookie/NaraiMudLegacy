...
function load_zone_index()
    ...
    for iz, zfn in ipairs(zflines) do
        ...
        Zone_index[iz] = { zname, renum_znum, zfn, wflines[iz],
...
function renum_zone_list()
    ...
    zf = io.open(RENUMDIR .. 'zone/zone_files', 'w')
    wf = io.open(RENUMDIR .. 'world/world_files', 'w')
...
function renum_mob(num)
    ...
    if zone_number() > 0 and out_of_zone(nr)  then
        ...
        buf = string.format('OOZ       MOB: #%-6d "%s"  @ %s (%d) ' ,
          num, name, zone_name(), zone_base())
        print(buf)
    end
...
function check_zone(zoneinfo)
    ...
    if G_renum and plusok(renum_info[4]) and filename then
        ...
        if strok(filename) then
          zoneinfo[3] = 'zone/' .. filename .. '.zon'
          zoneinfo[4] = 'world/' .. filename .. '.wld'
          renum_zfile = zoneinfo[3]
        else
    ...
    while true do
        ...
        if ch == COMCHAR  then
          local ch2 = ln:sub(2,2)
          if strok(ch2) and string.find('MEGORPD', ch2) then
              ln = ln:sub(2,-1)
              ch = ch2
              com = true
          else
        ...
        if ch == 'G' or ch == 'R' then    -- 3 args
            a1, a2, a3, rest = ln:match('^%u%s+(%d+)%s+(%d+)%s+(%d+)(.*)')
            a4 = ''
        elseif string.find('MEOPD', ch) then
