...
void cast_group_sanctuary( byte level, struct char_data *ch, char *arg, int type,
                    struct char_data *victim, struct obj_data *tar_obj )
{
    switch (type) {
    case SPELL_TYPE_SPELL:
    case SPELL_TYPE_SCROLL:
    case SPELL_TYPE_STAFF:
        spell_group_sanctuary(level, ch, 0, 0);
        break;
    default : 
        log("Serious screw-up in group_sanctuary!");
        break;
    }
}
                    ...
                    struct char_data *victim, struct obj_data *tar_obj )
{
    switch (type) {
    case SPELL_TYPE_SPELL:
        spell_paralysis(level, ch, victim, 0);
        break;
    case SPELL_TYPE_POTION:
        spell_paralysis(level, ch, ch, 0);
        break;
    case SPELL_TYPE_SCROLL:
        if(victim)
            spell_paralysis(level, ch, victim, 0);
    ...
    case SPELL_TYPE_STAFF:
        ...
        break;
    default : 
        log("Serious screw-up in paralysis!");
        break;
    }
}
