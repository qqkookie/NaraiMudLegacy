...
int is_ok(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
    if (shop_index[shop_nr].open1>time_info.hours){
        ...
        if (shop_index[shop_nr].open2>time_info.hours){
            ...
            do_say(keeper,
                   "Sorry, come back tomorrow.", COM_SAY);
            return(FALSE);
        };
    ...
    {
        do_say(keeper,
               "I don't trade with someone I can't see!", COM_SAY);
        return(FALSE);
    };
                   ...
                   struct char_data *keeper, int shop_nr)
{
    ...
    GET_GOLD(keeper) += (int)(temp1->obj_flags.cost*
                              shop_index[shop_nr].profit_buy) / SHOP_COST_MUL ;
                    ...
                    struct char_data *keeper,int shop_nr)
{
    ...
    if (GET_GOLD(keeper)*SHOP_COST_MUL <(int) (temp1->obj_flags.cost*
    ...
    GET_GOLD(ch) += (int) (temp1->obj_flags.cost*
                           shop_index[shop_nr].profit_sell);
    GET_GOLD(keeper) -= (int) (temp1->obj_flags.cost*
                               shop_index[shop_nr].profit_sell)/SHOP_COST_MUL ;
...
int shop_keeper(struct char_data *ch, int cmd, char *arg)
{
    ...
    if((cmd== COM_LIST )&&(ch->in_room==real_room(shop_index[shop_nr].in_room))){
        /* List */
