...
/* ************************************************************************
   *  file: cast2.c , Implementation of magic.             Part of DIKUMUD *
		...
		int type, struct char_data *tar_ch, struct obj_data *tar_obj)
{
	...
	if (affected_by_spell(ch, SPELL_DETECT_INVISIBLE))
	    return;
	spell_detect_invisible(level, ch, ch, 0);
	break;
    case SPELL_TYPE_STAFF:
	for (tar_ch = world[ch->in_room].people;
	     tar_ch; tar_ch = tar_ch->next_in_room)
	...
	break;
    default:
	log("Serious screw-up in detect invisible!");
	break;
    }
}
...
void cast_invisible(byte level, struct char_data *ch, char *arg, int type,
		       struct char_data *tar_ch, struct obj_data *tar_obj)
{
    switch (type) {
    case SPELL_TYPE_SPELL:
...
void cast_protect_from_align(byte level, struct char_data *ch, char *arg, int type,
			 struct char_data *tar_ch, struct obj_data *tar_obj)
{
    switch (type) {
    case SPELL_TYPE_SPELL:
	spell_protect_from_align(level, ch, tar_ch, 0);
	break;
    case SPELL_TYPE_POTION:
	spell_protect_from_align(level, ch, ch, 0);
	break;
    case SPELL_TYPE_SCROLL:
	if (tar_obj)
	    return;
	...
	break;
    default:
	log("Serious screw-up in protect from align!");
	break;
    }
}
		  ...
		  struct char_data *tar_ch, struct obj_data *tar_obj)
{
    ...
    sprintf(tmpbuf, "%s is tried cast reraise.", GET_NAME(ch));
    log(tmpbuf);
    switch (type) {
    case SPELL_TYPE_SPELL:
