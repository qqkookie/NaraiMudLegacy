...
int neverland(struct char_data *ch, int cmd, char *arg)
{
    ...
    if (cmd != CMD_DOWN)  /* specific to Room 14167. cmd 6 is move down */
	return (FALSE);
    // NOTE: to Room 14170-141744. (14174 is no exit room)
...
int dark_sand(struct char_data *ch, int cmd, char *arg)
{
    ...
    if (!IS_NPC(ch) && (cmd == CMD_NORTH || cmd == CMD_EAST || cmd == CMD_SOUTH
			|| cmd == CMD_WEST || cmd == CMD_UP || cmd == CMD_DOWN)) {
	send_to_char
	    (".... 검은 모래에서 풍기는 기운이 힘을 빠지게합니다.\n\r", ch);
	drain = dice(4, 4);
	if (GET_HIT(ch) < 150)
	    drain = drain / 2;
	if (GET_HIT(ch) < 60)
	...
	if (GET_MOVE(ch) > 0)
	    GET_MOVE(ch) -= dice(3, 4);
    }
    else if (!IS_NPC(ch) && (cmd == CMD_CAST)) {
	GET_MANA(ch) -= dice(3, 3);
    }
    return FALSE;
}
...
int school_cold(struct char_data *ch, int cmd, char *arg)
{
	...
	lev = GET_LEVEL(vict);
	if (IS_NPC(vict) || lev > 9)
	    continue;	/* helping low level player */

	if (GET_COND(vict, FULL) < 10 ) {
	    GET_COND(vict, FULL) = 10; GET_COND(vict, THIRST) = 10;
	}

	alive = 100 * GET_HIT(vict) / GET_MAX_HIT(vict);	// are they  hurt ?
	if (alive < 90) {
	    act("$n님이 손을 저어 따스한 기운을 만들어 냅니다.", 1, ch, 0, 0, TO_ROOM);
	    if ( alive < 50 && lev < 6)
		act("$n님이 '저런, 많이 다치셨군요' 라고 말합니다.", 1, ch, 0, 0, TO_ROOM);
	...
	if (!affected_by_spell(vict, SPELL_ARMOR)) {
	    if (!armored) {
		act("$n님의 몸에서 바람이 일더니 꽃향기가 되어 방안에 가득 퍼집니다.", 1, ch, 0, 0, TO_ROOM);
	...
	if (!affected_by_spell(vict, SPELL_BLESS)) {
	    if (!blessed) {
		act("$n님이 그의 반지에서 눈부신 빛살을 일으킵니다.", 1, ch, 0, 0, TO_ROOM);
...
int school_gracia(struct char_data *ch, int cmd, char *arg)
{
    ...
    if (cmd) {
	if (cmd == CMD_SAY || cmd == CMD_QUOTE)
	    gracia_response = G_SAY;	/* say */
	if (cmd == CMD_WHISPER)	/* whisper */
	    gracia_response = G_WHISPER;
	if (cmd == CMD_SMILE) { /* smile */
	...
	case G_WHISPER:
	    if (number(0, 1) == 0)
		act("Gracia님이 '흠..' 하고 말합니다", 1, ch, 0, 0,
		    TO_ROOM);
	    else
		act("$n님이 곰곰이 생각 합니다.", 1, ch, 0, 0,
		    TO_ROOM);
	    break;
	case G_SMILE:
	    if (gracia_lastperson != NULL &&
		gracia_lastperson->in_room == ch->in_room) {
		act("$n님이 $N님을 보고 미소를 짓습니다.", FALSE,
		    ch, 0, gracia_lastperson, TO_NOTVICT);
		send_to_char
		    ("Gracia님이 당신을 보고 방긋 웃습니다.\r\n",
		     gracia_lastperson);
		act("$n님의 눈가에 웃음이 번집니다.", 1, ch, 0, 0,
		    TO_ROOM);
	    }
	    break;
	}
    ...
    if ( gracia_response == G_WHISPER || (n_person + number(0, 4) < 2)) {
	/* whisper to */
	if (response_to != NULL && response_to->in_room == ch->in_room) {
	    sprintf(buf,
		    "Gracia님이 당신에게 '%s' 라고 속삭입니다.\r\n",
		    whisper_to[number(0, 2)]);
	    act("$n님이 $N님의 귀에 작은 소리로 무엇인가 말합니다.", FALSE, ch, 0, response_to, TO_NOTVICT);
	    send_to_char(buf, response_to);
	}
    }
    else if (n_person + number(0, 4) > 4) {
	sprintf(buf, "Gracia님이 '%s' 라고 말합니다.",
		say_all[number(0, 3)]);
...
static struct char_data *nara_lastperson;
int school_nara(struct char_data *ch, int cmd, char *arg)
{
    extern void advance_level(struct char_data *ch, int levup);
	...
	if (cmd == CMD_BOW) {	/* bow */
		...
		if (nara_lastperson == NULL) {
		    nara_lastperson = ch;
		    send_to_char("여기서 학위 수여 순서를 기다리세요.\r\n",
				 nara_lastperson);
		}
	    }
	}
	...
	if (!is_solved_quest(nara_lastperson, QUEST_SCHOOL)) {
	    strcpy(buf, "잘 하셨습니다. 당신은 우수한 성적으로 학업을 마쳤습니다.\r\n");
	    ...
	    if (IS_SET(nara_lastperson->specials.act, PLR_NOSHOUT)) {
		REMOVE_BIT(nara_lastperson->specials.act, PLR_NOSHOUT);
	    }

	    if (GET_LEVEL(nara_lastperson) < 2) {
		advance_level(nara_lastperson, 1);
	    ...
	    if ((obj = read_object(SCHOOL_OBJ_DIPLOMA, VIRTUAL)) == 0) {	/* Diploma */
		log("graduation_hall: fail to find diploma");
		return FALSE;
	    }
	    GET_GOLD(nara_lastperson) += 3000;
	    send_to_char("Nara님이 당신에게 우등 장학금을 수여했습니다.\r\n",
	    	nara_lastperson);
	    /* 0 is direction north if (EXIT(nara_lastperson, 0)) {
	       obj_to_room(obj, EXIT(nara_lastperson, 0)->to_room) ; } */
	    obj_to_char(obj, nara_lastperson);
	    // TODO: give recall or not?
	}
	else {		/* he/she solved already */
	    act("$n님이 $N님에게 '예의가 바른 학생이군' 이라 말합니다.", FALSE, ch, 0, nara_lastperson, TO_NOTVICT);
	    send_to_char("Nara님이 당신을 칭찬 합니다.\r\n", nara_lastperson);
	}
    }
    else {		/* he/she isn't here */
	act("Nara님이 '아니 아까 그 학생이 어디에 갔지 ?' 하고 말합니다.", 1, ch, 0, nara_lastperson, TO_ROOM);
    }
...
int sumiae_healer(struct char_data *ch, int cmd, char *arg)
{
	...
	if (ch->in_room != ch->specials.fighting->in_room) {
		...
		mob_teleport(ch->specials.fighting, GET_LEVEL(ch->specials.fighting));
	...
	if (!IS_NPC(vict) && !strncmp(vict->player.name, memory_name, 254)) {
		...
		if (number(1, 10) >= 3) {
		    ...
		    act("$n utters the words 'Deep Thanatos'.", 1, ch, 0, 0, TO_ROOM);
		    cast_sleep(GET_LEVEL(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
		    cast_paralysis(GET_LEVEL(ch), ch, "", SPELL_TYPE_SPELL, vict, 0);
		    return (TRUE);
		}
	    }
	    else {
	...
	if (!IS_NPC(vict)) {	/* helping player */
	    if (GET_LEVEL(vict) < IMO + 1)
		num_of_pcs++;

	    lev = GET_LEVEL(vict);
	    if (lev > 25)
		continue;	/* only help low level */
	...
	if (n < 10)
	    ...
	    act("$n bows deeply.", TRUE, ch, 0, 0, TO_ROOM);
	if ((++n) == 32)
	    n = 0;
	return TRUE;
    }
...
int is_wearing_teleport_ring(struct char_data *ch)
{
	...
	if (obj_index[ch->equipment[WEAR_FINGER_R]->item_number].virtual == 2711)
	    teleport_ring = 1;
    }
    if (ch->equipment[WEAR_FINGER_L]) {
	if (obj_index[ch->equipment[WEAR_FINGER_L]->item_number].virtual == 2711)
	    teleport_ring = 1;
    }
...
int control_center(struct char_data *ch, int cmd, char *arg)
{
	...
	if (cmd == CMD_LOOK) {
		...
		act("$n tried to touch machine, but failed", FALSE, ch, 0, 0, TO_ROOM);
	    }
...
int bogan(struct char_data *ch, int cmd, char *arg)
{
	...
	if (sobj && number(0, 5) > 1) {	/* found something - steal */
	    ...
	    stash_char(vict, 0);
	    dir = number(0, 5);
	    if (CAN_GO(ch, dir) && !IS_SET(world[EXIT(ch, dir)->to_room].room_flags, NO_MOB))
