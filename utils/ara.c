...
/* file id */
char fileid[] = "@(#)cliso3.c	1.16 94/11/06" ;
		...
		if ( local_s >= 0 ) {
			...
			if ( FD_ISSET(local_s, &in_set)) {
				...
				if ((nread = get_from_local(msg, local_s)) < 0 ) {
					...
					pr_msg(filmsg) ;
#endif
					send_remote_connection(msg, nread, remote_s) ;
					}
				}
			}	/* end of if local_s >= 0 */
		}	/* end of for */
...
int open_remote_connection(int new_s)
{
	...
	if ( person % 2 == 0 ) {
		...
		serv_addr.sin_addr.s_addr = inet_addr(ARA_HOST) ;
		serv_addr.sin_port = htons(ARA_PORT) ;
		}
